# Distributed Datacenter Blueprint — Spine-Leaf with DCI & VMware

> **Architecture:** Dual-site spine-leaf fabric · Redundant DCI interconnect · VMware vSphere compute
> **Layout:** Tier-aligned rows — all same-tier nodes across DC1 and DC2 share the same horizontal level
> **Renderer:** Mermaid v11 `flowchart TB` — open with `Cmd+K V` in VS Code
...
## Layout Strategy: Tier-Aligned Rows

The key structural change from the previous version is inverting the subgraph hierarchy:

| Previous layout | This layout |
|---|---|
| Outer: `DC1` and `DC2` containers | Outer: `SPINE_ROW`, `DCI_ROW`, `LEAF_ROW`, `COMPUTE_ROW` |
| Inner: tier layers inside each DC | Inner: `DC1` and `DC2` halves inside each tier row |
| DC1 left / DC2 right (whole DC) | All spines in one row · All leaves in next row |
| Tiers stack vertically per DC | **Same-tier nodes always share the same horizontal band** |

By making the **tier the outer container** and **DC the inner split**, Mermaid's `direction LR` on each tier subgraph ensures DC1 nodes always appear left of DC2 nodes at the same vertical rank.

## Visual Legend

| Color | Shape | Role |
|---|---|---|
| **Purple** `#4A148C` | Rectangle | Spine switches — L3 fabric backbone |
| **Orange** `#BF360C` | **Hexagon** | DCI Gateway leaves — inter-site role |
| **Blue** `#01579B` | Rectangle | Compute leaf switches — ToR server-facing |
| **Green** `#1B5E20` | **Cylinder** | VMware ESXi hosts |
| **Orange line 6px** | `== DCI ==` | DCI backbone — dual redundant inter-site |
